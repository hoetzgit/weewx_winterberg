...
try:
    ...
    def logmsg(level, msg):
        syslog.syslog(level, 'restx: MQTT2: %s' % msg)
    def logdbg(msg):
...
class MQTT2(weewx.restx.StdRESTbase):
    def __init__(self, engine, config_dict):
        ...
        Default is None
        """
        super(MQTT2, self).__init__(engine, config_dict)
        loginf("service version is %s" % VERSION)
        site_dict = weewx.restx.get_site_dict(config_dict, 'MQTT2', 'server_url')
        if not site_dict:
            return
        ...
        if 'tls' in config_dict['StdRESTful']['MQTT2']:
            site_dict['tls'] = dict(config_dict['StdRESTful']['MQTT2']['tls'])

        if 'inputs' in config_dict['StdRESTful']['MQTT2']:
            site_dict['inputs'] = dict(config_dict['StdRESTful']['MQTT2']['inputs'])
...
class MQTTThread(weewx.restx.RESTThread):
    ...
    def get_mqtt_client(self):
        ...
        try:
            ...
            logerr('Failed to connect to MQTT2 server (%s): %s' %
...
#               _obfuscate_password(self.server_url))
    ...
    def process_record(self, record, dbm):
        ...
        if not self.mc:
            raise weewx.restx.FailedPost('MQTT2 client not available')
