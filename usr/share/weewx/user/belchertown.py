...
# [!!!!] Anpassungen im Code durch This Manhart
...
#        logdbg("get_extension_list [0] weewx.debug=" + str(weewx.debug) + " belchertown_debug=" + str(belchertown_debug))           # [!!!!]
...
#                logdbg("XXXX [1] t=" + chart_dict[chartgroup]["title"])    # [!!!!]
            else:
                graphpage_titles[chartgroup] = chartgroup
#                logdbg("XXXX [2] t=" + chartgroup)                         # [!!!!]
...
#        logdbg("get_extension_list [2] NOAA")           # [!!!!]
...
#        logdbg("get_extension_list [2] before forecast forecast_enabled=" + self.generator.skin_dict["Extras"]["forecast_enabled"] + " id=" + self.generator.skin_dict["Extras"]["forecast_api_id"])           # [!!!!]
        if (
...
#            logdbg("forecast [0] id=" + forecast_api_id)                                                                   # [!!!!]
            ...
            if forecast_is_stale:
                try:
                    ...
                    if "forecast_dev_file" in self.generator.skin_dict["Extras"]:
                        ...
                        # Current conditions
#                        logdbg("forecast [1] url=" + forecast_current_url)                                   # [!!!!]
            ...
            if (
                ...
                visibility = "N/A"
                visibility_unit = ""
#                logdbg("get_extension_list [3] forecast else")           # [!!!!]
        else:
            current_obs_icon = ""
            current_obs_summary = ""
            visibility = "N/A"
            visibility_unit = ""
            cloud_cover = ""
#            logdbg("get_extension_list [4] forecast else")           # [!!!!]

#        logdbg("get_extension_list [5] after forecast")           # [!!!!]
...
#                logdbg("XXXX [1] t=" + plot_options.get("title", ""))                                           # [!!!!]
...
#                logdbg("XXXX [3] t=" + chart_title)                                           # [!!!!]
                ...
                for line_name in self.chart_dict[chart_group][plotname].sections:
                    ...
                    if time_length == "today":
                        ...
                        t_dt = datetime.datetime.fromtimestamp(minstamp)                                                                    # [!!!!]
                        output[chart_group][plotname]["options"]["title"] = t_dt.strftime("%B %Y")                                          # [!!!!] Monat und Jahr im Titel
#                        logdbg("XXXX [4] min=" + str(minstamp) + "/" + timestamp_to_string(minstamp) + " max=" + str(maxstamp))             # [!!!!]
                    elif time_length == "years_ago":
                        minstamp, maxstamp = archiveYearSpan(
                            timespan.stop, years_ago=time_ago
                        )
                        t_dt = datetime.datetime.fromtimestamp(minstamp)                                                                    # [!!!!]
                        output[chart_group][plotname]["options"]["title"] = t_dt.strftime("%Y")                                             # [!!!!] Jahr im Titel
                        ...
                        l = len(output[chart_group][plotname]["options"]["xAxis_categories"])                                                      # [!!!!]
                        if l > 0:
                            tmin_dt = datetime.datetime.fromtimestamp(minstamp)                                                                    # [!!!!]
                            tmax_dt = datetime.datetime.fromtimestamp(maxstamp)                                                                    # [!!!!]
                            s1 = " (" + tmin_dt.strftime("%d.%m") + " - " + tmax_dt.strftime("%d.%m") + ")"                                        # [!!!!]
                            s2 = output[chart_group][plotname]["options"]["title"]                                                                 # [!!!!]
                            if s2.find(s1) == -1:                                                                                                  # [!!!!]
                                output[chart_group][plotname]["options"]["title"] = s2 + s1                                                        # [!!!!]
#                        logdbg("XXXX [5] min=" + str(minstamp) + "/" + timestamp_to_string(minstamp) + " max=" + str(maxstamp) + " l=" + str(l))      # [!!!!]
                        ...
                        if year_specific == 2019:                                                                          # [!!!!] 221211
                            minstamp = start_ts                                                                            # [!!!!] 221211
                        else:                                                                                              # [!!!!] 221211
                            x_dt = datetime.datetime.strptime(str(year_specific) + "-01-01 00:00:00","%Y-%m-%d %H:%M:%S")  # [!!!!] 221211
                            x_ts = int(time.mktime(x_dt.timetuple()))                                                      # [!!!!] 221211
                            minstamp = x_ts                                                                                # [!!!!] 221211
#                            logdbg("XXXX minstamp=%s, y=%s" % (str(minstamp),str(year_specific)))                   # [!!!!] 221211
#                        minstamp = start_ts                                                                               # [!!!!] 221211 (deaktiviert)
                        maxstamp = stop_ts
                    else:
...
#            logdbg("Interval is: %s" % aggregate_interval)
        ...
        if xAxis_groupby or len(xAxis_categories) >= 1:
            ...
            if time_length == "all":
                xxx = startOfDay(archive.firstGoodStamp())                     # [!!!!] 221211
#                logdbg("XXXX xxx=%s, start_ts=%s" % (str(xxx),str(start_ts)))
                if xxx == start_ts:                                            # [!!!!] 221211
                    start_ts = startOfDay(archive.firstGoodStamp()) + 86400    # [!!!!] 221211
                else:                                                          # [!!!!] 221211
                    start_ts = start_ts + 86400                                # [!!!!] 221211
